...
object Sites {
  ...
  implicit val addressFmt = Json.format[Site]
  ...
  def all: Future[List[Site]] = {
    val futureList: Future[List[Site]] = Couchbase.bucket.searchValues[Site]("Moorfields", "moorfields")(new Query().setIncludeDocs(true)).toList
    futureList
  }

  def create = {
    val document = Site("foo", "bar", "blah", "baz")
